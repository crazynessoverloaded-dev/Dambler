      ...
      <div style={{ background: '#0f1118', minHeight: '100vh' }}>
        <div style={{ maxWidth: 860, margin: '0 auto', padding: '48px 24px 80px' }}>
          ...
          <div style={{ marginBottom: 36 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
              <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: 10, display: 'flex' }}>
                <AlertTriangle style={{ width: 18, height: 18, color: 'rgba(255,255,255,0.6)' }} />
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
            style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: '20px 24px', marginBottom: 20 }}>
            <div style={{ display: 'flex', gap: 14 }}>
              <ShieldAlert style={{ width: 20, height: 20, color: 'rgba(255,255,255,0.5)', flexShrink: 0, marginTop: 1 }} />
              <div>
                <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.5)', marginBottom: 7, textTransform: 'uppercase', letterSpacing: 0.8 }}>Important Warning</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05 }} style={card}>
                <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.4)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.7 }}>{item.label}</p>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }} style={{ ...card, marginBottom: 24 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.4)', marginBottom: 16, textTransform: 'uppercase', letterSpacing: 0.8 }}>Signs of Problem Gambling</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 20px' }}>
              {SIGNS.map((sign, i) => (
                <div key={i} style={{ display: 'flex', gap: 9, alignItems: 'flex-start' }}>
                  <span style={{ width: 4, height: 4, borderRadius: '50%', background: 'rgba(255,255,255,0.25)', flexShrink: 0, marginTop: 8 }} />
            ...
            {TOOLS.map((tool, i) => {
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.24 + i * 0.06 }} style={card}>
                  <Icon style={{ width: 17, height: 17, color: 'rgba(255,255,255,0.45)', marginBottom: 10 }} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            {RESOURCES.map((r, i) => {
              const Icon = r.icon;
              return (
                <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.34 + i * 0.06 }} style={card}>
                  <Icon style={{ width: 16, height: 16, color: 'rgba(255,255,255,0.45)', marginBottom: 10 }} />
                  <p style={{ fontSize: 13, fontWeight: 700, color: '#fff', marginBottom: 5 }}>{r.title}</p>
                  <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', fontFamily: 'monospace', marginBottom: 4 }}>{r.detail}</p>
                  <p style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.35)', margin: 0 }}>{r.sub}</p>
                </motion.div>
              );
            })}
          </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
            ...
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.35)', margin: 0 }}>
              ...
              <span style={{ color: 'rgba(255,255,255,0.6)' }}>Settings → Responsible Gambling</span>
              {' '}or email{' '}
              <span style={{ color: 'rgba(255,255,255,0.6)' }}>rg@dambler.com</span>
            </p>
          </motion.div>
        </div>
      </div>
